...
function validateTemplate(template: string): void {
  for (const match of template.matchAll(ANY_BRACE)) {
    ...
    for (const spec of expr.slice(bodyStart).split(',')) {
      ...
      if (name === '') {
        throw new Error(`Empty variable spec in template: ${template}`);
      }
      if (name.includes('*') || name.includes(':')) {
        throw new Error(`Unsupported modifier in '${name}': explode (*) and prefix (:N) are not supported`);
      }
      if (!VARNAME.test(name)) {
        throw new Error(`Invalid variable name '${name}' in template: ${template}`);
      }
    }
  }
}
...
export class TemplateEngine {
  expand(template: string, variables: Record<string, string>): string {
    ...
    for (const match of template.matchAll(ANY_BRACE)) {
      ...
      const names = body.split(',').map((s) => s.trim());
