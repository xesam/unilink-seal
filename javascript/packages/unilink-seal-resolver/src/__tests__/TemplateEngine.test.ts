...
describe('TemplateEngine', () => {
  ...
  describe('rejects whitespace in variable specs (mirrored in the Java SDK)', () => {
    const engine = new TemplateEngine();

    it('rejects spaces around a variable name', () => {
      expect(() => engine.expand('{ userId }', { userId: 'x' })).toThrow("Invalid variable name");
    });

    it('rejects a space after the comma in a multi-variable spec', () => {
      expect(() => engine.expand('{a, b}', { a: '1', b: '2' })).toThrow("Invalid variable name");
    });

    it('rejects spaces in a query expression', () => {
      expect(() => engine.expand('{?a, b}', { a: '1', b: '2' })).toThrow("Invalid variable name");
    });

    it('rejects in variableNames too', () => {
      expect(() => engine.variableNames('{ userId }')).toThrow("Invalid variable name");
    });
  });

  describe('rejects unbalanced braces (mirrored in the Java SDK)', () => {
    const engine = new TemplateEngine();

    it('rejects an unclosed brace', () => {
      expect(() => engine.expand('https://x.com/{', { a: '1' })).toThrow("Unclosed '{'");
    });

    it('rejects an unmatched closing brace', () => {
      expect(() => engine.expand('https://x.com/}', { a: '1' })).toThrow("Unmatched '}'");
    });

    it('rejects nested braces', () => {
      expect(() => engine.expand('https://x.com/{{a}}', { a: '1' })).toThrow("Nested '{'");
    });

    it('rejects in variableNames too', () => {
      expect(() => engine.variableNames('https://x.com/{')).toThrow("Unclosed '{'");
    });
  });

  describe('typed error codes (mirrored in the Java SDK)', () => {
    const engine = new TemplateEngine();

    it('throws ResolutionError/UNSUPPORTED_TEMPLATE for out-of-subset operators and modifiers', () => {
      for (const bad of ['{+path}', '{#frag}', '{.x}', '{/x}', '{;x}', '{?list*}', '{?x:3}']) {
        const error = (() => {
          try {
            engine.expand(bad, {});
          } catch (e) {
        ...
        expect(error.code === 'UNSUPPORTED_TEMPLATE' || error.code === 'INVALID_TEMPLATE').toBe(true);
      }
    });

    it('throws ResolutionError/INVALID_TEMPLATE for bad names and braces', () => {
      for (const bad of ['{ userId }', '{a, b}', 'https://x.com/{', 'https://x.com/}', '{bad-name}']) {
        const error = (() => {
          try {
            engine.expand(bad, {});
          } catch (e) {
